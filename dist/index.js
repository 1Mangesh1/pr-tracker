...
function getCacheKey(prId, title) {
...
async function handleRequest(request, env) {
    ...
    try {
        ...
        if (method === "GET" && pathname === "/health") {
            return new Response(JSON.stringify({ status: "ok", timestamp: new Date().toISOString() }), { status: 200, headers: { "Content-Type": "application/json" } });
        }
        ...
        if (method === "POST" && pathname === "/api/fetch-prs") {
            ...
            if (!query?.trim()) {
                return new Response(JSON.stringify({ success: false, error: "Search query required" }), { status: 400, headers: { "Content-Type": "application/json" } });
            }
            try {
                const token = getGitHubToken(env);
                const prs = await fetchPRsFromGitHub(query, token, maxResults);
                return new Response(JSON.stringify({ success: true, data: prs, message: `Fetched ${prs.length} PRs` }), { status: 200, headers: { "Content-Type": "application/json" } });
            }
            catch (error) {
                console.error("GitHub fetch error:", error);
                return new Response(JSON.stringify({ success: false, error: error instanceof Error ? error.message : "Failed to fetch PRs" }), { status: 500, headers: { "Content-Type": "application/json" } });
            }
        }
        // Summarize single PR
        if (method === "POST" && pathname === "/api/summarize") {
            ...
            if (!title || !repoName) {
                return new Response(JSON.stringify({ success: false, error: "Title and repoName required" }), { status: 400, headers: { "Content-Type": "application/json" } });
            }
            try {
                ...
                return new Response(JSON.stringify({ success: false, error: error instanceof Error ? error.message : "Summarization failed" }), { status: 500, headers: { "Content-Type": "application/json" } });
            }
        }
        ...
        if (method === "POST" && pathname === "/api/batch-process") {
            ...
            try {
                ...
                return new Response(JSON.stringify({ success: true, data: response, message: `Processed ${processed}/${prs.length} PRs` }), { status: 200, headers: { "Content-Type": "application/json" } });
            }
            catch (error) {
                console.error("Batch process error:", error);
                return new Response(JSON.stringify({ success: false, error: error instanceof Error ? error.message : "Batch processing failed" }), { status: 500, headers: { "Content-Type": "application/json" } });
            }
        }
        // Not found
        ...
        return new Response(JSON.stringify({ success: false, error: error instanceof Error ? error.message : "Internal error" }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
}
// Main export
export default {
